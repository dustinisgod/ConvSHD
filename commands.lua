...
local function setToggleOption(option, value, name)
    ...
    elseif value == "off" then
        ...
        print("Usage: /convSHD " .. name .. " on/off")
    end
end
...
local function setNumericOption(option, value, name)
    if value == "" then
        print("Usage: /convSHD " .. name .. " <number>")
        return
    end
...
local function setChaseOnOff(value)
    if value == "" then
        print("Usage: /convSHD Chase <targetName> <distance> or /convSHD Chase off/on")
    ...
    elseif value == 'off' then
        ...
        if not targetName then
            print("Invalid input. Usage: /convSHD Chase <targetName> <distance>")
            return
        end
        ...
        if not distance then
            print("Invalid distance provided. Usage: /convSHD Chase <targetName> <distance> or /convSHD Chase off")
            return
        end
...
local function setCampHere(value1)
    ...
    elseif tonumber(value1) then
        ...
        print("Error: Invalid command. Usage: /convSHD camphere <distance>, /convSHD camphere on, /convSHD camphere off")
    end
end
...
local function setMeleeOptions(meleeOption, stickOption, stickDistance)
    ...
    elseif meleeOption == "front" or meleeOption == "behind" then
        ...
        elseif stickOption then
            print("Invalid stick distance. Usage: /convSHD melee front/behind <distance>")
        end
    else
        print("Error: Invalid command. Usage: /convSHD melee on/off or /convSHD melee front/behind <distance>")
    end
end
...
local function setTankIgnore(scope, action)
    ...
    elseif action == "remove" then
        ...
        print("Error: Invalid action. Usage: /convSHD tankignore zone/global add/remove")
    end
end
...
local function setTankMelee(value)
    ...
    elseif value == "off" then
        ...
        print("Usage: /convSHD tankmelee on/off")
    end
end
...
local function setAssistMelee(value)
    ...
    elseif value == "off" then
        ...
        print("Usage: /convSHD assistmelee on/off")
    end
end

local function setUsePet(value)
    setToggleOption("usePet", value, "Use Pet")
end
...
local function setFeignDeath(value)
    setToggleOption("feignDeath", value, "Feign Death")
end
...
local function commandHandler(command, ...)
    ...
    elseif command == "pet" then
        setUsePet(args[1])
    elseif command == "buffson" then
        setBuffsOn(args[1])
    elseif command == "feigndeath" then
...
function commands.init()
    -- Single binding for the /convSHD command
    mq.bind('/convSHD', function(command, ...)
